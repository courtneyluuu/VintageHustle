...
window.onload = () => {
  ...
  const trendingProductList = document.getElementById("trending-product-list");
  ...
  const productListHTML = productList
    .filter((item) => item.category.includes("trending"))
    .map(
      (product) => `
      <a href="/order.html">
  ...
  trendingProductList.innerHTML = productListHTML;
