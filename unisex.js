...
window.onload = () => {
  ...
  const unisexProductList = document.getElementById("unisex-product-list");
  ...
  const productListHTML = productList
    .filter((item) => item.category.includes("unisex"))
    .map(
      (product) => `
      <a href="/order.html">
  ...
  unisexProductList.innerHTML = productListHTML;
